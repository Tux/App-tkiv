...
App::tkiv - An image viewer based on IrfanView

=head1 SYNOPSIS

 $ iv [-f] [option=value ...] [dir]

 $ iv -f imageposition=s thumsorting=size .

=head1 DESCRIPTION

C<iv> is a pure-perl application that was written to mimic the behaviour
...
which iv was initially modelled
  ...
  keys_quit
    ...
    q Q ESC		Quit program		Close image,
						return to thumbs
  ...
  keys_nextpic
    ...
    Space Right Down	Next pic (round robin)	id
...
=item Slideshow
 ...
 - behaviour: location, dir depth, cycling
...
=item Titles and decoration behaviour
...
Copyright (C) 2004-2009 H.Merijn Brand for PROCURA B.V.
